...
const Explorer = forwardRef(({ onSelect, dirtree, setDirtree }, ref) => {
  ...
  function nodeFromPath(path) {
    ...
    for (let pathPartIndex = 0; pathPartIndex < pathParts.length; pathPartIndex++) {
      const child = (node.children || []).find(n => n.path.split('/').at(-1) === pathParts[pathPartIndex]);
  ...
  async function addFile({ props: { node: parentNode = dirtree } }) {
    ...
    parentNode.children.push({ name, path: joinPaths(parentNode.path, name) });
    setDirtree({ ...dirtree });
  ...
  async function addFolder({ props: { node: parentNode = dirtree } }) {
    ...
    parentNode.children.push({ name, path: joinPaths(parentNode.path, name), children: [] });
    setDirtree({ ...dirtree });
    await localforage.setItem('dirtree', dirtree);
  }
  ...
  return (
    <>
      <div ref={ref} className="Explorer" onContextMenu={e => showContextMenu(e)}>
